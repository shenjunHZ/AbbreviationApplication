...
namespace SqliteDB
{
    ...
    /************************************update interface*******************************************************/
    bool AbbreviationTable::updateDefinition(uint64_t iPrimaryKey, std::string strDefinition)
    {
        char szSelectSql[1024] = { 0 };

        dsl::DStr::sprintf_x(szSelectSql, sizeof(szSelectSql),
            "UPDATE %s SET DEFINITION = '%s' WHERE ID = %lu;", m_strTableName.c_str(), strDefinition.c_str(), iPrimaryKey);
        return m_pSqliteDB->ExecCmdText(szSelectSql);
    }
